...
The Display System is a component of the HoloViz MCP that enables AI assistants to create and manage Python visualizations through a dedicated web server. This document explains the architecture, design decisions, and key concepts.
...
The Display Server runs independently - you start it manually with the `display-server` CLI command. The MCP server connects to it via HTTP when you use the `holoviz_display` tool.
...
- `DISPLAY_SERVER_PORT`: Port number (default: 5005)
- `DISPLAY_SERVER_HOST`: Host address (default: 127.0.0.1)
