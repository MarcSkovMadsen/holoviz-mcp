...
In your IDE or development environment make sure to [start the HoloViz MCP server](../tutorials/getting-started/#start-the-server).
...
1. Make sure the MCP server is running (`holoviz-mcp` command)
...
1. Check that `holoviz-mcp` is still running in your terminal
2. Look for "Panel server started successfully" in the startup logs
3. Try restarting: stop `holoviz-mcp` (Ctrl+C) and start it again

### Visualization shows an error

If the visualization page shows an error message:

1. Click to expand the error details
2. Check if required packages are installed (e.g., seaborn, matplotlib)
3. Try a simpler visualization first to verify the system is working
4. Ask your AI to fix the code based on the error message

### Need more help?

- Check the [troubleshooting guide](../how-to/troubleshooting.md)
- Review [configuration options](../how-to/configuration.md)
- Read about the [Display System architecture](../explanation/display-system.md)
