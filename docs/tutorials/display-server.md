...
This foundation will enable you to effectively use the HoloViz MCP [`holoviz_display`](display-mcp-tool.md) tool and integrate visualization capabilities into your AI-assisted workflows.
...
The Display Server is included with the HoloViz MCP package. Let's install it:

```bash
pip install holoviz-mcp
```

This will install the Display Server along with all necessary dependencies.
...
**Example scenario:** If you try to create a visualization with:
...
And see an error about `sklearn`, install it with `pip install scikit-learn` and resubmit your code.
...
- **[Use the Display Server with the MCP tool](./display-mcp-tool.md)** - Integrate the Display Server with AI assistants
- **[Learn about the Display System architecture](../explanation/display-system.md)** - Understand how the Display Server works under the hood
