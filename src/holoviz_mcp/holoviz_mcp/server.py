...
SHOW_RESOURCE_URI = "ui://holoviz-mcp/show.html"
SHOW_TEMPLATE_PATH = Path(__file__).parent / "templates" / "show.html"
SHOW_PYODIDE_RESOURCE_URI = "ui://holoviz-mcp/show-pyodide.html"
SHOW_PYODIDE_TEMPLATE_PATH = Path(__file__).parent / "templates" / "show_pyodide.html"


def _externalize_display_url(url: str) -> str:
    """Convert local display URLs to externally reachable proxy/Codespaces URLs."""
    if not url:
        return url
    ...
    proxy_base = os.getenv("JUPYTER_SERVER_PROXY_URL") or config.server.jupyter_server_proxy_url
    if proxy_base:
...
async def display(
    ...
    try:
        ...
        url = _externalize_display_url(response.get("url", ""))
...
async def show_pyodide(
    ...
    display-server execution should be avoided. It returns a payload consumed
