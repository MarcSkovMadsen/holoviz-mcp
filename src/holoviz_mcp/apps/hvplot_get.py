...
An interactive version of the holoviz_mcp.hvplot_mcp.server.hvplot_get tool.
"""
...
class GetDocstringConfiguration(param.Parameterized):
    ...
    async def _update_plot_types(self):
        ...
        try:
            result = await call_tool("hvplot_list", {})
    ...
    async def _update_result(self):
        ...
        try:
            result = await call_tool("hvplot_get", params)
...
class HvplotGetDocstringApp(pn.viewable.Viewer):
    ...
    title = param.String(default="HoloViz MCP - hvplot_get Tool Demo")
