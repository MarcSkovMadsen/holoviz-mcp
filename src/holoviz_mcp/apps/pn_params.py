...
An interactive version of the holoviz_mcp.panel_mcp.server.pn_params tool.
"""
...
- Lighter weight than `pn_get` (no docstring/signature)
- Best for **understanding configuration options**

### Difference from pn_get

| Feature | pn_get | pn_params |
|---------|--------|-----------|
...
class GetComponentParametersConfiguration(param.Parameterized):
    """Configuration for Panel pn_params tool."""
    ...
    async def _update_result(self):
        """Execute pn_params and update result."""
        self.loading = True
        self.error_message = ""
        self.result = {}
        ...
        try:
            result = await call_tool("pn_params", params)
...
class PanelGetComponentParametersApp(pn.viewable.Viewer):
    ...
    title = param.String(default="HoloViz MCP - pn_params Tool Demo")
