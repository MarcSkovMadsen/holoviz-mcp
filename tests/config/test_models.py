...
class TestDocsConfig:
    ...
    def test_docs_config_with_repositories(self):
        ...
        config = DocsConfig(
            repositories={
                "test-repo": GitRepository(url="https://github.com/test/repo.git", base_url="https://example.com/", url_transform="/")  # type: ignore
            }
        )
        assert "test-repo" in config.repositories
        assert str(config.repositories["test-repo"].url) == "https://github.com/test/repo.git"
        assert str(config.repositories["test-repo"].url_transform) == "/"
